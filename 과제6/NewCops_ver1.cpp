...
public:
    ...
    void setDirection(){
        int dx = nextNode.pX - currentNode.pX, dy = nextNode.pY - currentNode.pY;;
        if(dx == 0 && dy > 0)
            direction = TOP;
        else if(dx == 0 && dy < 0)
            direction = BOTTOM;
    ...
    bool isCrush(Node& object){
        if(inTheNode())return 0;
        if((object.location.pX == location.pX)&& abs(object.location.pY - location.pY)<=1) return 1;
        if((object.location.pY == location.pY) && abs(object.location.pX - location.pX)<=1) return 1;
        return 0;
    }
